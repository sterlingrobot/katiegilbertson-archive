...
Dataface.modules.DataGrid.buildColumn = function(fielddef, colName){
	...
	if ( dFormat != null ){
		if ( typeof(fielddef['DataGrid.format']) != 'undefined' && fielddef['DataGrid.format'] ){
			dFormat = fielddef['DataGrid.format'];
		}
		out.renderer = function(v){
			if ( v instanceof Date ){
				return v.format(dFormat);
			} else {
				return v;
			}
		};
	...
	var renderer = out.renderer;
	out.renderer = function(v,p,r){
		
		if ( r.get(colName+'__edit') ){
			p.css += 'DataGrid-editable';
			
		} else {
			p.css += 'DataGrid-noneditable';
			
		}
		
		if ( typeof(renderer) == 'function' ){
			return renderer(v,p,r);
		} else {
			return v;
		}
	};
	...
	switch (fielddef.widget.type){
		case 'text':
			...
			if ( fielddef.Type.match(/date|timestamp/) ){
				...
				//alert("Time field: "+options.interval);
...
Dataface.modules.DataGrid.getEditorOptions = function(fielddef){
	...
	if ( fielddef.Type.match(/datetime|timestamp/) ){
		...
		options.format = 'H:i';
		if ( fielddef.widget.interval ){
			options.increment = parseInt(fielddef.widget.interval);
		}
		if ( fielddef.widget.format ){
			options.format = fielddef.widget.format;
		}
		
		options.renderer = function(v){
			if ( v instanceof Date ){
				return v.format(options.format);
			} else {
				return v;
			}
		};
		
		if ( fielddef.widget.starttime ){
			options.minValue = fielddef.widget.starttime;
		}
		
		if ( fielddef.widget.endtime ){
			options.maxValue = fielddef.widget.endtime;
		}
	}
	return options;
};
...
Dataface.modules.DataGrid.getRecord = function(fielddefs){
	var params = [];
	for ( i in fielddefs ){
		params.push(this.getRecordColumn(fielddefs[i], i));
		params.push({name: i+'__view'});
		params.push({name: i+'__edit'});
	}
...
Dataface.modules.DataGrid.create = function(params){
		...
		params.tbar =  [{
			...
			handler: function(){
				var dlg = new xataface.RecordDialog({
					table: jQuery('#editor-grid').attr('data-table'),
    ...
    grid.addListener('beforeedit', function(o){
    	if ( !o.record.get(o.field+'__edit') ) return false;
    	var opts = [];
    	for ( var i in o ){
    		opts.push(i+':'+o[i]);
    	}
    ...
    grid.addListener('rowmousedown', function(o){
