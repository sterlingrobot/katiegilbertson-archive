...
$result = mysql_query("SHOW fields from ".$db_prefix."jobs LIKE '".$field."'");
@$rows = mysql_num_rows($result);

if (empty($rows)) {
$passwd_query = mysql_query("ALTER TABLE ".$db_prefix."jobs ADD $field VARCHAR(25) NOT NULL;");
...
if (empty($rows)) {
$passwd_query = mysql_query("ALTER TABLE ".$db_prefix."jobs ADD $field VARCHAR(50) NOT NULL;");
echo "              <tr><td width=10 class=table_rows style='padding-left:25px;color:#FF9900;font-weight:bold;'>Added</td><td class=table_rows 
                      align=left>:&nbsp;<b>$field</b> field has been added to the <u>jobs</u> table.</td></tr>\n";
$passed_or_not = "1";
} 
...
if (empty($rows)) {
$passwd_query = mysql_query("ALTER TABLE ".$db_prefix."jobs ADD $field VARCHAR(75) NOT NULL;");
echo "              <tr><td width=10 class=table_rows style='padding-left:25px;color:#FF9900;font-weight:bold;'>Added</td><td class=table_rows 
                      align=left>:&nbsp;<b>$field</b> field has been added to the <u>jobs</u> table.</td></tr>\n";
$passed_or_not = "1";
} 
...
if (empty($rows)) {
$passwd_query = mysql_query("ALTER TABLE ".$db_prefix."jobs ADD $field TINYINT(1) NOT NULL default '0';");
echo "              <tr><td width=10 class=table_rows style='padding-left:25px;color:#FF9900;font-weight:bold;'>Added</td><td class=table_rows 
                      align=left>:&nbsp;<b>$field</b> field has been added to the <u>jobs</u> table.</td></tr>\n";
$passed_or_not = "1";
} 
...
$result = mysql_query("SHOW FIELDS FROM ".$db_prefix."jobs");
while ($row = mysql_fetch_array($result)) {
  $name = "".$row["Field"]."";
  $type = "".$row["Type"]."";
  ...
  if (($name == 'jobname') && ($type != 'varchar(50)')) {
    $alter_result = mysql_query("ALTER TABLE ".$db_prefix."jobs CHANGE jobname jobname VARCHAR(50) NOT NULL");
  ...
  if (($name == 'tstamp') && ($type != 'bigint(14)')) {
    $alter_result = mysql_query("ALTER TABLE ".$db_prefix."jobs CHANGE tstamp tstamp BIGINT(14) DEFAULT NULL");
    echo "              <tr><td width=10 class=table_rows style='padding-left:25px;color:#0000FF;font-weight:bold;'>Changed</td><td class=table_rows 
...
if (isset($recreate_admin)) {
    ...
    if ($recreate_admin == '1') {
        ...
        if (!isset($admin_user)) {
            $add_admin_query = mysql_query("INSERT INTO ".$db_prefix."jobs 
...
$emp_tstamp_result = mysql_query("update ".$db_prefix."jobs set tstamp = (unix_timestamp(tstamp) - '".$gmt_offset."')");
