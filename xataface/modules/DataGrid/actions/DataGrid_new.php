...
class actions_DataGrid_new {
	function handle(&$params){
		...
		foreach ( $table->graftedFields(true) as $fieldName => $fieldDef ){
			$options[$fieldName] = $fieldName.' (Read Only)';
		...
		$mod = Dataface_ModuleTool::loadModule('modules_DataGrid');
		$mod->registerSkin();
		$json = new Services_JSON;
		$app->addHeadContent('<style type="text/css"> select { width: 250px !important;}</style>');
